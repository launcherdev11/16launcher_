...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, 
                            ...
                            QSlider, QMessageBox, QTabWidget, QFrame, QStackedWidget, QCheckBox, QScrollArea, QTextEdit)
...
class NewsTab(QWidget):
    ...
    def setup_ui(self):
        ...
        # Editor Tab (only visible in edit mode)
        self.editor_tab = QWidget()
        self.setup_editor_tab()
        self.tabs.addTab(self.editor_tab, "Редактор")
        self.tabs.setTabVisible(2, False)  # Скрываем по умолчанию
        
        layout.addWidget(self.tabs)
        
        # Кнопка редактирования (только если файл news.json существует)
        self.edit_button = QPushButton("Редактировать новости")
        self.edit_button.clicked.connect(self.toggle_edit_mode)
        self.edit_button.setVisible(os.path.exists(NEWS_FILE))
    ...
    def setup_editor_tab(self):
        ...
        self.cancel_button.clicked.connect(self.toggle_edit_mode)
    ...
    def load_minecraft_news(self):
        try:
            # Получаем новости с официального сайта Minecraft
    ...
    def load_launcher_news(self):
        try:
            if os.path.exists(NEWS_FILE):
                ...
                # Создаем файл с примером новости, если его нет
                example_news = [
                    {
                        "title": "Добро пожаловать в 16Launcher!",
                        "date": datetime.now().strftime("%d.%m.%Y"),
                        "content": "Это ваш персональный лаунчер Minecraft. Здесь будут появляться новости и обновления."
                    }
                ]
                with open(NEWS_FILE, 'w', encoding='utf-8') as f:
                    json.dump(example_news, f, ensure_ascii=False, indent=4)
                
                self.load_launcher_news()
                
        except Exception as e:
            self.launcher_news_list.setText(f"Не удалось загрузить новости лаунчера: {str(e)}")
            logging.error(f"Ошибка загрузки новостей лаунчера: {str(e)}")
    
    def toggle_edit_mode(self):
        if self.tabs.isTabVisible(2):  # Если редактор уже виден
            self.tabs.setTabVisible(2, False)
            self.edit_button.setText("Редактировать новости")
        else:
            # Загружаем текущие новости в редактор
            try:
                if os.path.exists(NEWS_FILE):
                    with open(NEWS_FILE, 'r', encoding='utf-8') as f:
                        news = json.load(f)
                    self.editor.setPlainText(json.dumps(news, ensure_ascii=False, indent=4))
                else:
                    self.editor.setPlainText("[]")
            except Exception as e:
                self.editor.setPlainText(f"Ошибка загрузки: {str(e)}")
            
            self.tabs.setTabVisible(2, True)
            self.edit_button.setText("Закрыть редактор")
    
    def save_news(self):
        try:
            news = json.loads(self.editor.toPlainText())
            
            # Проверяем структуру
            if not isinstance(news, list):
                raise ValueError("Новости должны быть массивом")
            
            for item in news:
                if not all(key in item for key in ['title', 'date', 'content']):
                    raise ValueError("Каждая новость должна содержать title, date и content")
            
            # Сохраняем
            with open(NEWS_FILE, 'w', encoding='utf-8') as f:
                json.dump(news, f, ensure_ascii=False, indent=4)
            
            QMessageBox.information(self, "Успех", "Новости успешно сохранены!")
            self.load_launcher_news()
            self.toggle_edit_mode()
            
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Не удалось сохранить новости: {str(e)}")
...
class SettingsTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.close_on_launch_checkbox = QCheckBox("Закрывать лаунчер при запуске игры (в разработке)", self)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.start_btn = None
        self.skin_head_label = None
        self.change_name_btn = None
        self.load_skin_btn = None
    ...
    def setup_game_tab(self):
        """Настройка компактной вкладки запуска игры"""
        layout = QVBoxLayout(self.game_tab)
        layout.setContentsMargins(15, 15, 15, 10)  # Уменьшенные отступы
        layout.setSpacing(8)  # Минимальный промежуток между элементами

        # 1. Блок профиля (аватар + имя)
        profile_layout = QHBoxLayout()
        
        # Миниатюра скина (48x48)
        self.skin_head_label = QLabel()
        self.skin_head_label.setFixedSize(48, 48)
        self.skin_head_label.setStyleSheet("""
            background-color: #444; 
            border-radius: 5px;
            border: 1px solid #555;
        """)
        profile_layout.addWidget(self.skin_head_label)
        ...
        welcome_label.setStyleSheet("font-size: 14px; color: #aaa;")
        name_layout.addWidget(welcome_label)

        # Поле для ввода имени
        self.username = QLineEdit()
        self.username.setPlaceholderText('Введите имя')
        self.username.setMaximumWidth(180)
        ...
        # Кнопки действий (смена имени/скина)
        action_btn_layout = QHBoxLayout()
        action_btn_layout.setSpacing(4)
        
        # Кнопка смены имени
        self.change_name_btn = QPushButton()
        self.change_name_btn.setIcon(QIcon(resource_path("assets/edit_icon.png")))
        self.change_name_btn.setIconSize(QSize(16, 16))
        self.change_name_btn.setFixedSize(24, 24)
        self.change_name_btn.setToolTip("Случайное имя")
        self.change_name_btn.setStyleSheet("QPushButton { border: none; padding: 0; }")
        self.change_name_btn.clicked.connect(self.set_random_username)
        action_btn_layout.addWidget(self.change_name_btn)

        # Кнопка смены скина
        self.load_skin_btn = QPushButton()
        self.load_skin_btn.setIcon(QIcon(resource_path("assets/skin_icon.png")))
        self.load_skin_btn.setIconSize(QSize(16, 16))
        self.load_skin_btn.setFixedSize(24, 24)
        self.load_skin_btn.setToolTip("Сменить скин")
        self.load_skin_btn.setStyleSheet("QPushButton { border: none; padding: 0; }")
        self.load_skin_btn.clicked.connect(self.load_skin)
        action_btn_layout.addWidget(self.load_skin_btn)
        
        name_layout.addLayout(action_btn_layout)
        ...
        # Строка выбора версии
        version_layout = QHBoxLayout()
        version_layout.setSpacing(6)
        
        self.version_select = QComboBox()
        self.version_select.setMinimumHeight(32)
        self.version_select.setStyleSheet("QComboBox { padding: 3px; }")
        ...
        version_layout.addWidget(self.favorite_btn)
        ...
        self.loader_select.setMinimumHeight(32)
        self.loader_select.setStyleSheet("QComboBox { padding: 3px; }")
        self.loader_select.addItem("Vanilla", "vanilla")
        self.loader_select.addItem("Forge", "forge")
        self.loader_select.addItem("Fabric", "fabric")
        launch_layout.addWidget(self.loader_select)

        self.start_btn = QPushButton("Играть")
        ...
        # 3. Прогресс-бар (скрыт по умолчанию)
        self.start_progress = QProgressBar()
        self.start_progress.setFixedHeight(6)
        self.start_progress.setTextVisible(False)
        self.start_progress.setVisible(False)
        layout.addWidget(self.start_progress)

        # Инициализация
        self.update_version_list()  # Загрузка списка версий
        self.update_skin_preview()  # Загрузка превью скина
        self.username.textChanged.connect(self.update_skin_preview)  # Обновление при изменении имени
        
    def update_skin_preview(self):
        """Обновляет превью головы скина"""
        username = self.username.text().strip()
        if not username:
            self.skin_head_label.clear()
            return
        
        skin_path = os.path.join(SKINS_DIR, f"{username}.png")
        if os.path.exists(skin_path):
            # Создаем миниатюру головы (8x8 пикселей из верхнего левого угла)
            skin = QPixmap(skin_path)
            head = skin.copy(8, 8, 8, 8).scaled(64, 64, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            self.skin_head_label.setPixmap(head)
        else:
            self.skin_head_label.clear()
                
    def update_version_list(self):
        """Обновляет список версий в зависимости от выбранного типа"""
        if not hasattr(self, 'version_select') or not self.version_select:
            return
        ...
        versions = get_version_list()
        show_only_favorites = hasattr(self, 'version_type_select') and \
                            self.version_type_select and \
    ...
    def load_skin(self):
        ...
        if file_path:
            try:
                ...
                # Копируем скин в нужное место
                username = self.username.text().strip() or "default"
    ...
    def apply_dark_theme(self):
        ...
        QWidget {
            ...
            color: #f1f1f1;
                ...
                /* Слайдер памяти - стиль "игровой полосы" */
