...
class ModManager:
    ...
    @staticmethod
    def download_modrinth_mod(mod_id, version, parent_widget=None):
        """Скачивает мод с Modrinth с поддержкой отмены и продолжения"""
        try:
            ...
            for v in versions:
                if version in v['game_versions']:
                    ...
                    # Используем новый DownloadDialog для загрузки
                    if parent_widget:
                        dialog = DownloadDialog(file_url, dest_path, file_name, parent_widget)
    ...
    @staticmethod
    def download_curseforge_mod(mod_id, version, parent_widget=None):
        """Скачивает мод с CurseForge с поддержкой отмены и продолжения"""
        try:
            headers = {
...
class ModsTab(QWidget):
    ...
    def install_modrinth_mod(self, mod_id):
        ...
        try:
            ...
            # Устанавливаем мод
            success, message = ModManager.download_modrinth_mod(mod_id, version, self)
...
class ModLoaderInstaller(QThread):
    ...
    def cancel(self):
        ...
        if self.current_download_manager:
    ...
    def install_optifine(self):
        ...
        try:
            ...
            # Используем новый DownloadManager для загрузки
            self.current_download_manager = DownloadManager(download_url, optifine_path, f"OptiFine_{self.mc_version}.jar")
            self.current_download_manager.progress_signal.connect(lambda current, total, filename: 
                self.progress_signal.emit(current, total, f"Загрузка {filename}"))
            self.current_download_manager.status_signal.connect(lambda status: 
                self.progress_signal.emit(0, 0, status))
            self.current_download_manager.finished_signal.connect(self._on_optifine_download_finished)
            self.current_download_manager.start()
            
        except Exception as e:
            if not self.is_cancelled:
                self.finished_signal.emit(False, f"Ошибка установки OptiFine: {str(e)}")
    
    def _on_optifine_download_finished(self, success, message):
        """Обработчик завершения загрузки OptiFine"""
        if self.is_cancelled:
            return
        ...
        if success:
            ...
            if not self.is_cancelled:
                self.finished_signal.emit(False, f"Ошибка загрузки OptiFine: {message}")   
    ...
    def install_quilt_version(minecraft_version: str, loader_version: str, install_dir: str, callback: dict):
        """Установка Quilt"""
        quilt_profile = {
        ...
        version_dir = os.path.join(str(install_dir), "versions", f"quilt-loader-{loader_version}-{minecraft_version}")
        os.makedirs(version_dir, exist_ok=True)
        
        with open(os.path.join(version_dir, f"quilt-loader-{loader_version}-{minecraft_version}.json"), 'w') as f:
...
class LaunchThread(QThread):
    ...
    def run(self):
        try:
            ...
            if hasattr(self.parent_window, 'ely_session') and self.parent_window.ely_session:
                print("[LAUNCH THREAD] Applying Ely.by session...")
                options.update({
            ...
            if self.loader_type == "forge":
                print("[LAUNCH THREAD] Processing Forge version...")
                ...
                print(f"[LAUNCH THREAD] Forge launch version: {launch_version}")

            elif self.loader_type == "fabric":
                print("[LAUNCH THREAD] Processing Fabric version...")
                try:
                    loader_version = get_latest_loader_version()
                    launch_version = f"fabric-loader-{loader_version}-{self.version_id}"
                    print(f"[LAUNCH THREAD] Fabric launch version: {launch_version}")
                except Exception as e:
                    raise Exception(f"Fabric loader error: {str(e)}")

            elif self.loader_type == "quilt":
                print("[LAUNCH THREAD] Processing Quilt version...")
                ...
                install_minecraft_version(
                    ...
                    callback={
                        ...
                        'setProgress': lambda value: (
                            print(f"[INSTALL] Progress: {value}%"),
    ...
    def setup_authlib(self, options):
        ...
        options['jvmArguments'].append("-Dauthlibinjector.yggdrasil.prefetched={...}")  # Добавьте данные из docs.ely.by

    def download_authlib(self):
        """Скачивает authlib-injector"""
        try:
            response = requests.get("https://maven.ely.by/releases/by/ely/authlib/1.2.0/authlib-1.2.0.jar")  # Актуальная версия
...
class SettingsTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.setup_ui()
        # self.setup_language_selector()  # Удалено, чтобы не было дублирования
    ...
    def setup_ui(self):
        ...
        # Загрузки
        downloads_card = QWidget()
        downloads_card.setStyleSheet(card_style)
        downloads_layout = QVBoxLayout(downloads_card)
        downloads_layout.setSpacing(7)
        downloads_header = QLabel("Загрузки")
        downloads_header.setStyleSheet(header_style)
        downloads_layout.addWidget(downloads_header)
        
        # Кнопка проверки прерванных загрузок
        self.check_downloads_button = QPushButton("Проверить прерванные загрузки")
        self.check_downloads_button.setStyleSheet("""
            QPushButton {
                background-color: #3d3d3d;
                border: 1px solid #555555;
                border-radius: 5px;
        ...
        """)
        self.check_downloads_button.clicked.connect(self.check_resume_downloads)
        downloads_layout.addWidget(self.check_downloads_button)
    ...
    def check_resume_downloads(self):
        """Показывает диалог для проверки прерванных загрузок"""
        resume_download_dialog(self)
...
def download_authlib_injector(parent_widget=None):
    """Скачивает последнюю версию Authlib Injector с поддержкой отмены и продолжения"""
    try:
        ...
        # Используем новый DownloadDialog для загрузки
        if parent_widget:
            dialog = DownloadDialog(download_url, AUTHLIB_JAR_PATH, "authlib-injector.jar", parent_widget)
            if dialog.exec_() == QDialog.Accepted:
                return True
            else:
                return False
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Проверяем прерванные загрузки при запуске
        QTimer.singleShot(2000, self.check_resume_downloads_on_startup)
        
    def check_resume_downloads_on_startup(self):
        """Проверяет прерванные загрузки при запуске приложения"""
        candidates = check_resume_downloads()
        if candidates:
            reply = QMessageBox.question(self, "Прерванные загрузки", 
                                       f"Обнаружено {len(candidates)} неполных файлов.\n"
                                       "Хотите проверить и продолжить загрузки?",
    ...
    """
        ...
        if hasattr(self, 'theme_button'):
            self.theme_button.setIcon(QIcon(resource_path(f"assets/sun{icon_suffix}.png")))
        if hasattr(self, 'settings_button'):
            self.settings_button.setIcon(QIcon(resource_path(f"assets/set64{icon_suffix}.png")))
        if hasattr(self, 'news_button'):
            self.news_button.setIcon(QIcon(resource_path(f"assets/news64{icon_suffix}.png")))
        if hasattr(self, 'telegram_button'):
            self.telegram_button.setIcon(QIcon(resource_path(f"assets/tg{icon_suffix}.png")))
        if hasattr(self, 'support_button'):
            self.support_button.setIcon(QIcon(resource_path(f"assets/support64{icon_suffix}.png")))
        if hasattr(self, 'play_button'):
            self.play_button.setIcon(QIcon(resource_path(f"assets/play64{icon_suffix}.png")))
        if hasattr(self, 'toggle_sidebar_button'):
            is_visible = self.sidebar.isVisible()
            icon_name = "toggle_open" if is_visible else "toggle_close"
            self.toggle_sidebar_button.setIcon(QIcon(resource_path(f"assets/{icon_name}{icon_suffix}.png")))
        if hasattr(self, 'random_name_button'):
            self.random_name_button.setIcon(QIcon(resource_path(f"assets/random{icon_suffix}.png")))
        if hasattr(self, 'open_folder_button'):
            self.open_folder_button.setIcon(QIcon(resource_path(f"assets/folder{icon_suffix}.png")))
        ...
        if hasattr(self, 'settings_tab'):
            if hasattr(self.settings_tab, 'theme_button'):
                self.settings_tab.theme_button.setIcon(QIcon(resource_path(f"assets/sun{icon_suffix}.png")))
...
class DownloadManager(QThread):
    """Управляет загрузками с поддержкой отмены и продолжения"""
...
class DownloadDialog(QDialog):
    """Диалог загрузки с кнопками отмены и паузы"""
    def __init__(self, url, filepath, filename="", parent=None):
        super().__init__(parent)
        self.url = url
        self.filepath = filepath
    ...
    def setup_ui(self):
        ...
        self.title_label = QLabel(f"Загрузка: {self.filename}")
    ...
    def start_download(self):
        self.download_manager = DownloadManager(self.url, self.filepath, self.filename)
        ...
        # Запускаем таймер для обновления скорости
    ...
    def update_progress(self, current, total, filename):
        if total > 0:
            ...
            self.info_label.setText(f"{current_mb:.1f} MB / {total_mb:.1f} MB ({progress}%)")
            
            # Обновляем данные для расчета скорости
            self.last_bytes = current
    
    def update_speed(self):
        """Обновляет информацию о скорости загрузки"""
        if hasattr(self.download_manager, 'downloaded_bytes') and self.download_manager.downloaded_bytes > 0:
            current_time = time.time()
            time_diff = current_time - self.last_time
            if time_diff > 0:
                bytes_diff = self.download_manager.downloaded_bytes - self.last_bytes
                speed_mbps = (bytes_diff / time_diff) / (1024 * 1024)
                self.speed_label.setText(f"Скорость: {speed_mbps:.2f} MB/s")
                self.last_bytes = self.download_manager.downloaded_bytes
    ...
    def toggle_pause(self):
        if self.download_manager:
            if self.download_manager.is_paused:
                ...
                self.speed_label.setText("Загрузка приостановлена")
    ...
    def closeEvent(self, event):
        ...
        if self.download_manager and not self.download_manager.is_cancelled:
            reply = QMessageBox.question(self, "Подтверждение", 
                                       "Загрузка еще не завершена. Отменить загрузку?",
                                       QMessageBox.Yes | QMessageBox.No)
            if reply == QMessageBox.Yes:
...
class DownloadManager:
    """Глобальный менеджер загрузок для управления всеми загрузками"""
    
    def __init__(self):
        self.active_downloads = {}  # {download_id: DownloadManager}
        self.download_history = []  # История загрузок
        self.paused_downloads = {}  # Приостановленные загрузки
    
    def start_download(self, url, filepath, filename="", download_id=None):
        """Запускает новую загрузку"""
        if download_id is None:
            download_id = str(uuid1())
        
        download_manager = DownloadManager(url, filepath, filename)
        self.active_downloads[download_id] = download_manager
        ...
        return download_id, download_manager
    
    def pause_download(self, download_id):
        """Приостанавливает загрузку"""
        if download_id in self.active_downloads:
            download_manager = self.active_downloads[download_id]
            download_manager.pause()
            self.paused_downloads[download_id] = download_manager
            del self.active_downloads[download_id]
    
    def resume_download(self, download_id):
        """Возобновляет загрузку"""
        if download_id in self.paused_downloads:
            download_manager = self.paused_downloads[download_id]
    ...
    def cancel_download(self, download_id):
        ...
        if download_id in self.active_downloads:
            ...
            del self.active_downloads[download_id]
        elif download_id in self.paused_downloads:
            self.paused_downloads[download_id].cancel()
            del self.paused_downloads[download_id]
    ...
    def get_download_status(self, download_id):
        ...
        if download_id in self.active_downloads:
            ...
            return 'completed' if any(d['id'] == download_id for d in self.download_history) else 'unknown'
    
    def cleanup_completed_downloads(self):
        """Очищает завершенные загрузки"""
        completed_ids = []
        for download_id, download_manager in self.active_downloads.items():
            if download_manager.isFinished():
...
def check_resume_downloads():
    """Проверяет и предлагает продолжить прерванные загрузки"""
    resume_candidates = []
    
    # Проверяем папки загрузок на наличие неполных файлов
    download_dirs = [
        os.path.join(MINECRAFT_DIR, "mods"),
        os.path.join(MINECRAFT_DIR, "libraries"),
        os.path.join(MINECRAFT_DIR, "assets"),
        ...
        if os.path.exists(download_dir):
            for filename in os.listdir(download_dir):
                ...
                if os.path.isfile(filepath):
                    # Проверяем, не является ли файл неполным
                    file_size = os.path.getsize(filepath)
                    if file_size > 0 and file_size < 1024 * 1024:  # Файлы меньше 1MB могут быть неполными
                        # Проверяем расширение файла
                        if filename.endswith(('.jar', '.zip', '.mod')):
...
def resume_download_dialog(parent=None):
    """Показывает диалог для продолжения прерванных загрузок"""
    candidates = check_resume_downloads()
    ...
    dialog.setWindowTitle("Прерванные загрузки")
    ...
    # Заголовок
    header_label = QLabel("Обнаружены неполные загрузки:")
    ...
    for candidate in candidates:
        ...
        item_text = f"{candidate['filename']} ({size_mb:.2f} MB)"
    ...
    resume_button = QPushButton("Продолжить выбранные")
    resume_button.clicked.connect(lambda: handle_resume_selected(file_list, dialog))
    button_layout.addWidget(resume_button)
    
    delete_button = QPushButton("Удалить выбранные")
    delete_button.clicked.connect(lambda: handle_delete_selected(file_list, dialog))
...
def handle_resume_selected(file_list, dialog):
    """Обрабатывает продолжение выбранных загрузок"""
    selected_items = file_list.selectedItems()
    
    if not selected_items:
        QMessageBox.warning(dialog, "Предупреждение", "Выберите файлы для продолжения загрузки")
        return
    
    # Здесь можно добавить логику для определения URL файлов
    # и продолжения их загрузки
    QMessageBox.information(dialog, "Информация", 
                          f"Выбрано {len(selected_items)} файлов для продолжения загрузки.\n"
                          "Функция определения URL файлов требует дополнительной реализации.")
    
    dialog.accept()

def handle_delete_selected(file_list, dialog):
    """Обрабатывает удаление выбранных файлов"""
    selected_items = file_list.selectedItems()
    
    if not selected_items:
        QMessageBox.warning(dialog, "Предупреждение", "Выберите файлы для удаления")
        return
    
    reply = QMessageBox.question(dialog, "Подтверждение", 
                               f"Удалить {len(selected_items)} неполных файлов?",
                               QMessageBox.Yes | QMessageBox.No)
    ...
    if reply == QMessageBox.Yes:
        for item in selected_items:
            ...
            try:
                ...
                QMessageBox.warning(dialog, "Ошибка", f"Не удалось удалить {candidate['filename']}: {str(e)}")
        
        QMessageBox.information(dialog, "Успех", "Выбранные файлы удалены")
        dialog.accept()
...
if __name__ == "__main__":
    try:
        ...
        for value, message in steps:
            time.sleep(0.5)  # Замените на реальные операции
